<template>
  <div class="relative">
    ...
    <div class="relative bg-surface-container-lowest p-8 lg:p-10 rounded-2xl ghost-border space-y-7">
      <!-- Success State -->
      <div v-if="formState === 'success'" class="text-center py-10 space-y-4">
        ...
        <p class="text-on-surface-variant text-sm">{{ $t('contact.form.successText') }}</p>
        <button
          @click="resetForm"
          class="mt-4 px-6 py-2.5 rounded-xl text-sm font-semibold text-primary ghost-border hover:bg-surface-container-low transition-all"
        >
      ...
      <!-- Error State -->
      <div v-else-if="formState === 'error'" class="text-center py-10 space-y-4">
        <span class="material-symbols-outlined text-5xl text-error">error</span>
        <h3 class="text-xl font-bold text-on-surface">{{ $t('contact.form.errorTitle') }}</h3>
        <p class="text-on-surface-variant text-sm">{{ $t('contact.form.errorText') }}</p>
        <button
      ...
      <!-- Form -->
      <form v-else name="contact" method="POST" data-netlify="true" netlify-honeypot="bot-field" @submit.prevent="handleSubmit">
        ...
        <div class="space-y-7">
          ...
          <div class="space-y-2">
            ...
            <input
              ...
              class="w-full bg-surface-container-low border-0 rounded-xl py-3.5 px-4 text-on-surface placeholder:text-on-surface-variant/40 focus:ring-2 focus:ring-primary/20 focus:bg-surface-container-lowest transition-all outline-none"
            />
          </div>
          ...
          <div class="space-y-2">
            ...
            <textarea
              ...
              rows="4"
              class="w-full bg-surface-container-low border-0 rounded-xl py-3.5 px-4 text-on-surface placeholder:text-on-surface-variant/40 focus:ring-2 focus:ring-primary/20 focus:bg-surface-container-lowest transition-all resize-none outline-none"
            ></textarea>
          </div>
          ...
          <button
            ...
            class="w-full flex items-center justify-center gap-2 bg-gradient-to-br from-primary to-primary-container text-on-primary font-bold py-4 rounded-xl hover:shadow-lg hover:shadow-primary/20 transition-all active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed"
          >
      ...
      <template v-if="formState === 'idle' || formState === 'sending'">
        ...
        <div class="grid grid-cols-3 gap-3">
          <a
            ...
            class="flex flex-col items-center gap-2.5 py-4 rounded-xl bg-surface-container-low hover:bg-surface-container transition-all group"
          >
